...
;(function ($) {
    ...
    function setUp($el, settings) {
        ...
        $thumb = $el.find('.ytp-thumbnail').on('click', function (e) {
            ...
            if (!$el.hasClass(settings.video_loaded_class)) {
                $el.html('<iframe src="//www.youtube.com/embed/' + id + '?' + youtube_parameters + '&autoplay=1" frameborder="0"></iframe>')
        ...
        loadBackgroundImage(id, width, $thumb, youtube_data_url, img_bg);
    ...
    function loadBackgroundImage(id, width, $thumb, youtube_data_url, img_bg) {
      var thumb_img,
          thumb_url,
      ...
      downloadingImage.onload = function(data) {
        setBackgroundImage($thumb, this.src, img_bg);
      };
    ...
    function setBackgroundImage($thumb, url, img_bg) {
      // console.log(img_bg);
      if (img_bg) {
        $thumb.css({
            'background-image': ['url(', img_bg, ')'].join('')
